...
class LineIdentification:
    ...
    def edge_detection(self, frame, threshold=200, minLineLength=500, maxLineGap=100):
        ...
        if CALIBRATION_IMAGE_COUNTER < MAX_CALIBRATION_IMAGES:
            ...
            lines_list = self.extract_all_lines(copied_frame, threshold, minLineLength, maxLineGap)
        ...
        cv2.imshow("Calibrated image with lines", frame)
    ...
    def extract_side_line(self, frame, gray, candidate_pixels):
        ...
        print("Candidate Pixel Radiate Search Finished")
    ...
    def calibrate_camera(self, frame, gray, line_list, sideline_pixels):
        ...
        if line_list is not None and sideline_pixels is not None:
            ...
            if top_sideline != (None, None) and bottom_sideline != (None, None):
                ...
                self.calibrate_pixels(top_sideline, sideline_pixels, gray, True)
                self.calibrate_pixels(bottom_sideline, sideline_pixels, gray, False)
    ...
    def find_top_bottom_sideline(self, line_list, rows, cols):
        ...
        return top_sideline, bottom_sideline
    ...
    def find_sideline_edges(self, sideline_pixels, c, start_from_top):
        ...
        if start_from_top:
            ...
            for r in range(rows - 1, middle_y, -1):
                if sideline_pixels[r, c] == 1:
                    if not in_sideline:
                        in_sideline = True
                        y_bottom_edge = r
    ...
    def calibrate_pixels(self, sideline, sideline_pixels, gray, start_from_top):
        ...
        for c in range(cols):
            ...
            middle_y = int(rows / 2)
            middle_x = int(cols / 2)
            y_top_edge, y_bottom_edge = self.find_sideline_edges(sideline_pixels, c, start_from_top)
            ...
            if y_bottom_edge is not None:
                bottom_distance = y - y_bottom_edge
            else:
                bottom_distance = cols * -1

            if top_distance < 0:
                distance = top_distance
                closest_edge = y_top_edge
            elif bottom_distance > 0:
                distance = bottom_distance
                closest_edge = y_bottom_edge
            else:
                continue
            ...
            for pixel_y in range(start, end):
                ...
                if pixel_y > middle_y:
                    vertical_intensity = pixel_y / closest_edge
                else:
